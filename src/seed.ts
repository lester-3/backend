...
export async function seedDatabase(): Promise<void> {
  ...
  if (emailCount === 0) {
    await Email.insertMany(sampleEmails.map((e, index) => ({
      ...e,
  ...
  const driveCount = await DriveFile.countDocuments();
  if (driveCount === 0) {
    await DriveFile.insertMany(sampleDriveFiles);
  ...
  if (profileCount === 0) {
    await Profile.create({ name: 'Me', email: 'me@nmail.com', phone: '', avatar_path: '' });
