...
import type { CreateEmailPayload } from '../types';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const emails = await Email.find(filter).sort({ created_at: -1 });
    res.json(emails);
  } catch (err) {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const payload: CreateEmailPayload = req.body;
...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (read !== undefined) updates.read = read ? 1 : 0;
    ...
    const email = await Email.findByIdAndUpdate(req.params.id, updates, { new: true });
    if (!email) {
      res.status(404).json({ error: 'Email not found' });
      return;
    }
...
router.get('/stats/folders', async (_req: Request, res: Response) => {
  try {
    const total = await Email.aggregate([
      { $group: { _id: '$folder', count: { $sum: 1 } } },
    ]);
    const unread = await Email.aggregate([
      { $match: { read: 0 } },
      { $group: { _id: '$folder', count: { $sum: 1 } } },
    ]);
